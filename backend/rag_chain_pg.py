...
from backend.runtime_paths import ENV_PATH, PDF_DIR
...
def ask_pg(question: str, session_id: str = "default") -> dict:
    ...
    try:
        ...
        # Lấy danh sách nguồn đã dùng ở lượt trước (nếu có)
        previous_sources = _session_contexts.get(f"{session_id}_sources", [])
        ...
        # Với câu hỏi follow-up: ưu tiên giữ nguyên nguồn tài liệu của lượt trước
        if is_follow_up and previous_sources:
            filtered = [
                r for r in results
                if r.get("metadata", {}).get("source", "unknown") in previous_sources
            ]
            if filtered:
                print(f"[RAG] Follow-up → ưu tiên {len(filtered)}/{len(results)} chunks từ nguồn trước đó")
                results = filtered
...
import uuid
from backend.db import save_tai_lieu
